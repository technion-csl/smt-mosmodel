...
def buildGroupsSequentially(orig_pebs_df, layouts_dir):
    ...
    for index, row in pebs_df.iterrows():
        ...
        min_page = max_page = page_number
        for k in range(25):
            page = min_page - 1
            if page < 0 or page in all_pages:
                break
            min_page -= 1
        for k in range(25):
            page = max_page + 1
            if page > last_page or page in all_pages:
                break
            max_page += 1
        query = pebs_df.query(
        ...
        if query['NUM_ACCESSES'].sum() < desired_weights[i]:
            continue
        query_df = query.sort_values('PAGE_NUMBER', ascending=True)
...
if args.layout == 'layout1':
    i = 1
    groups = buildGroupsSequentially(pebs_df, args.layouts_dir)
