...
class LayoutGenerator():
    ...
    def removeTailPagesFromEndpoint(self, endpoint, desired_real_coverage):
        pages, offset = LayoutGeneratorUtils.getLayoutHugepages(endpoint, self.exp_dir)
        df = self.pebs_df.query('PAGE_NUMBER in {pages}'.format(pages=pages))
        df = df.sort_values('TLB_COVERAGE', ascending=False)
        endpoint_coverage = df['TLB_COVERAGE'].sum()
        endpoint_real_coverage = self.state_log.getRealCoverage(endpoint)
        endpoint_pebs_coverage = self.state_log.getPebsCoverage(endpoint)
        endpoint_real_to_pebs_scale = endpoint_pebs_coverage / endpoint_real_coverage
        scaled_desired_coverage = endpoint_real_to_pebs_scale * desired_real_coverage

        print('[DEBUG]: trying to remove tail pages from the leftmost subgroup layout (or some other derived layout from it)')
        print('[DEBUG]: subgroup layout to remove from it: ' + endpoint)
        print('[DEBUG]: subgroup layout coverage: ' + str(endpoint_coverage))
        print('[DEBUG]: desired real coverage: ' + str(desired_real_coverage))
        print('[DEBUG]: scaled desired pebs coverage: ' + str(scaled_desired_coverage))

        assert scaled_desired_coverage < endpoint_coverage

        removed_pages = []
        total_weight = endpoint_coverage
        epsilon = 0.2
        ...
        print('[DEBUG]: total removed tail pages from the leftmost layout: ' + str(len(removed_pages)))
        print('[DEBUG]: new layout coverage: ' + str(new_pebs_coverage))
        ...
        if self.state_log.hasOnlyBaseLayouts():
            ...
            # last layout was incremented by > 3%
                ...
                if last_layout_method == 'right-tail':
                    ...
                    pages, pebs_coverage = self.removeTailPagesFromEndpoint(
